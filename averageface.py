...
def read_points(txt_path):
    ...
    for filePath in sorted(os.listdir(txt_path)):
            ...
            with open(os.path.join(path, filePath)) as file:
                for line in file:
                    ...
                    if len(x) > 0 and len(y) > 0:
                        tmp_points.append((int(x), int(y)))

            # Store array of points
            points_array.append(tmp_points)
...
def read_images(dir_path):
    ...
    for filePath in sorted(os.listdir(dir_path)):
        ...
        if filePath.endswith(".jpg"):
            # Read image found.
            tmp_img = cv2.imread(os.path.join(path, filePath))

            # Convert to floating point
            tmp_img = np.float32(tmp_img) / 255.0

            # Add to array of images
            images_array.append(tmp_img)
...
def similarityTransform(inPoints, outPoints):
    ...
    tform = list(tform)
    # pip install opencv-python==3.4.2.16 // in your Terminal ,,, opencv-python-4.5.2.52
...
if __name__ == '__main__':
    ...
    avgface_proj_root = os.path.abspath(os.path.dirname(__file__))
    path = avgface_proj_root+'/../cspeople/scraped/insta_image3'
    ...
    avg_img = "images/insta_averageface.jpg"
    cv2.imwrite(avg_img, 255 * output)
    print('Saved image to {}'.format(avg_img))
    # cv2.waitKey(0)
